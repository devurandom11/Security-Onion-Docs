...
OPNsense is a free and open firewall that can be found at <https://opnsense.org/>. Similar to [pfSense](pfsense.md), there are a few different ways in which you can integrate OPNsense into your Security Onion deployment. First, you can configure OPNsense to send firewall logs to your Security Onion deployment. You can also configure OPNsense to send [NetFlow](netflow.md) data to your Security Onion deployment. Finally, a third option is to integrate OPNsense's Suricata into Security Onion so that it pulls [NIDS](nids.md) rules from your Security Onion deployment and then sends those Suricata alerts to Security Onion. All three options are detailed below.
...
You can integrate your OPNsense firewall with your Security Onion Grid system to utilize it as a makeshift sensor, allowing management of Suricata [NIDS](nids.md) rules through [Detections](detections.md). This is particularly useful for organizations that find it challenging to install a network TAP on their egress connection.
...
The [Detections](detections.md) module can now manage [NIDS](nids.md) rules for external Suricata instances. Please refer to the [NIDS](nids.md) section for detailed instructions on how to set this up.
