...
Zeek logs are sent to [Elasticsearch](elasticsearch.md) for parsing and storage and can then be found in [Dashboards](dashboards.md), [Hunt](hunt.md), and [Kibana](kibana.md). Here's an example of Zeek logs in [Hunt](hunt.md):

![Image](images/56_hunt.png)

## Community ID

Security Onion enables Zeek's built-in support for [Community ID](community-id.md).

## Packet Loss and Capture Loss

Zeek reports both packet loss and capture loss and you can find graphs of these in [InfluxDB](influxdb.md). If Zeek reports packet loss, then you most likely need to adjust the number of Zeek workers as shown below or filter out traffic using [BPF](bpf.md). If Zeek is reporting capture loss but no packet loss, this usually means that the capture loss is happening upstream in the TAP or SPAN port itself.

## Configuration

You can configure Zeek by going to [Administration](administration.md) --> Configuration --> Zeek. 

![Image](images/config-item-zeek.png)

Zeek logs are consumed by the Elastic Agent (managed by Elastic Fleet) so if you want to configure which Zeek logs are excluded, you can go to [Administration](administration.md) --> Configuration --> elasticfleet --> logging --> Zeek --> excluded.

![Image](images/config-item-elasticfleet.png)

## HOME_NET

The HOME_NET variable defines the networks that are considered home networks (those networks that you are monitoring and defending). The default value is RFC1918 private address space (10.0.0.0/8, 192.168.0.0/16, and 172.16.0.0/12). You can modify this default value by going to [Administration](administration.md) --> Configuration --> Zeek --> config --> networks --> HOME_NET.

## Performance

Zeek uses [AF-PACKET](af-packet.md) so that you can spin up multiple Zeek workers to handle more traffic.  
...
You can modify Zeek worker count by going to [Administration](administration.md) --> Configuration --> Zeek --> config --> node --> workers.

## Disabling

Zeek can be disabled by going to [Administration](administration.md) --> Configuration --> Zeek --> enabled.

## Syslog

To forward Zeek logs to an external syslog collector, please see the [syslog-output](syslog-output.md) section.

## Logs

Zeek logs are stored in `/nsm/zeek/logs`. They are collected by [Elastic Agent](elastic-agent.md), parsed by and stored in [Elasticsearch](elasticsearch.md), and viewable in [Dashboards](dashboards.md), [Hunt](hunt.md), and [Kibana](kibana.md).

We configure Zeek to output logs in JSON format. If you need to parse those JSON logs from the command line, you can use [jq](jq.md).
...
In addition to Zeek's default logs, we also include protocol analyzers for STUN, TDS, and Wireguard traffic. We also include support for ICS/SCADA protocols such as BACnet, BSAP, CIP, COTP, DNP3, ECAT, ENIP, Modbus, OPC UA, Profinet, and S7. All of these analyzers are enabled by default and you can find corresponding dashboards for each of them in [Dashboards](dashboards.md).

We also include MITRE BZAR scripts and you can read more about them at <https://github.com/mitre-attack/bzar>. Please note that the MITRE BZAR scripts are disabled by default. If you would like to enable them, you can do so via [Administration](administration.md) --> Configuration --> Zeek. Once enabled, you can then check for BZAR detections by going to [Dashboards](dashboards.md) and selecting the Zeek Notice dashboard.

As you can see, Zeek log data can provide a wealth of information to the analyst, all easily accessible through [Dashboards](dashboards.md), [Hunt](hunt.md), or [Kibana](kibana.md).

## File Extraction

By default, Zeek will extract files from network traffic and [Strelka](strelka.md) will then analyze those extracted files.

## VLAN Tags

If your network traffic has VLAN tags, then Zeek will log them in conn.log. [Dashboards](dashboards.md) includes a VLAN dashboard which shows this data.
