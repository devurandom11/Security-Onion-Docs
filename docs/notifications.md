...
### ElastAlert 2 Alerters
 ...
 [ElastAlert](elastalert.md) includes a large number of alerters that can reach out to remote systems to deliver notifications. As each alerter supports a unique protocol the alerter requires its own set of supporting parameters in order for the alerter to know how to reach out to the remote endpoint. For example, to send a notification to a Slack channel, a webhook URL must be provided.

Navigate to the [Administration](administration.md) -> Configuration screen. Next, locate the `ElastAlert` settings.

![Image](images/config-item-elastalert-alerter.png)

Notice there are special settings for Jira and SMTP notifications. These are unique in that [ElastAlert](elastalert.md) requires those two alerters to read their credentials from a file. Security Onion has simplified this process by presenting these Configuration fields to enter the optional credential data, and the backend process will take care of generating the required files for [ElastAlert](elastalert.md).

The files subtree includes a list of several file settings, which allows for populating the contents of certain files that the alerters can optionally utilize. Most alerters use the files for specifying a custom Certificate Authority, so that [ElastAlert](elastalert.md) can securely and confidently connect to remote servers that may be using custom SSL/TLS certificates. Again, Security Onion's backend process will handle generating these files from the supplied configuration data provided in the user interface.

Next, the **SOC > config > server > modules > elastalertengine > Notifications: Sev 0/Default Parameters** setting is used to customize each alerter's own parameters. As [ElastAlert](elastalert.md) already provides detailed documentation on the required parameters for each alerter, this documentation will not cover the same information, but instead will focus on two popular alerters: Slack and SMTP.
...
To have [Sigma](sigma.md) rules send notifications to Slack, add the following line to the **SOC > config > server > modules > elastalertengine > Notifications: Sev 0/Default Parameters** configuration setting:
...
To have [Sigma](sigma.md) rules send notifications via email, add the following lines to the **SOC > config > server > modules > elastalertengine > Notifications: Sev 0/Default Parameters** configuration setting:
...
Once the alerter parameters are configured, as described above, the next step is to configure [Detections](detections.md) in order to activate one or more notification alerters.

Navigate to the [Administration](administration.md) -> Configuration screen. Next, locate the `SOC > config > server > modules > elastalertengine` settings.
...
Important! After activating (or removing) an alerter from this setting, the [ElastAlert](elastalert.md) engine must be fully updated. This can be done via the [Detections](detections.md) screen, under the Options dropdown.
