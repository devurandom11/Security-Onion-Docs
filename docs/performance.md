...
For best performance, CPU intensive processes like [Zeek](zeek.md) and [Suricata](suricata.md) should be pinned to specific CPUs. In most cases, you'll want to pin sniffing processes to the same CPU that your sniffing NIC is bound to. For more information, please see the Performance subsection in the appropriate [Suricata](suricata.md) and [Zeek](zeek.md) sections.
...
## Elastic
You will want to make sure that each part of the pipeline is operating at maximum efficiency.  Depending on your configuration, this may include [Elastic Agent](elastic-agent.md), [Logstash](logstash.md), [Redis](redis.md), and [Elasticsearch](elasticsearch.md).
