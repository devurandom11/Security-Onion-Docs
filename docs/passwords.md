...
Your default user account should have sudo permissions. Command-line utilities that require administrative access can be prefixed with `sudo`. For example, the [so-status](so-status.md) command requires administrative access so you can run it with `sudo` as follows:
...
Activate passwordless login for your [SOC](security-onion-console.md) user by clicking the user icon in the upper-right corner, clicking `Settings`, and then going to the `Security` tab. Scroll down to the `Security Keys` section and follow the provided instructions.
