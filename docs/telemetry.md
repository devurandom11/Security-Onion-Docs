...
The SOC Telemetry data originates from the [SOC](security-onion-console.md) browser running on the analyst workstation. The domains being accessed from the [SOC](security-onion-console.md) browser are:
...
Security Onion periodically checks for package updates to ensure the operating system (OS) and related applications are kept patched and updated. These updates are critical to protecting the OS and installed packages from recently exposed vulnerabilities. This is different from Security Onion product upgrades via [soup](soup.md), which is manually invoked. When the OS package updates begin, the request to the Security Onion repository server(s) can include a limited set of telemetry data.
