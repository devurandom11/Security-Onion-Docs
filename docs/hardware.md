...
The [Architecture](architecture.md) section should have helped you determine how many machines you will need for your deployment. This section will help you determine what kind of hardware specs each of those machines will need.
...
Please see the [Architecture](architecture.md) section for detailed deployment scenarios.
...
- RAM: Used for [Logstash](logstash.md), [Elasticsearch](elasticsearch.md), disk cache for Lucene, [Suricata](suricata.md), [Zeek](zeek.md), etc. The amount of available RAM will directly impact search speeds and reliability, as well as ability to process and capture traffic.
- Disk: Used for storage of indexed metadata. A larger amount of storage allows for a longer retention period. It is typically recommended to retain no more than 30 days of hot [Elasticsearch](elasticsearch.md) indices.

Please refer to the [Architecture](architecture.md) section for detailed deployment scenarios.

## Manager node with local log storage and search

In an enterprise distributed deployment, a manager node will store logs from itself and sensor nodes. It can also act as a syslog destination for other log sources to be indexed into [Elasticsearch](elasticsearch.md). An enterprise manager node should have 8 CPU cores at a minimum, 16-128GB RAM, and enough disk space (multiple terabytes recommended) to meet your retention requirements.
...
- RAM: Used for [Logstash](logstash.md), [Elasticsearch](elasticsearch.md), and disk cache for Lucene. The amount of available RAM will directly impact search speeds and reliability.
- Disk: Used for storage of indexed metadata. A larger amount of storage allows for a longer retention period. It is typically recommended to retain no more than 30 days of hot [Elasticsearch](elasticsearch.md) indices.
...
- CPU: Used to receive incoming events and place them into [Redis](redis.md). Used to run all the front end web components and aggregate search results from the search nodes.
- RAM: Used for [Logstash](logstash.md) and [Redis](redis.md).
...
Search nodes increase search and retention capacity with regard to [Elasticsearch](elasticsearch.md). These nodes parse and index events, and provide the ability to scale horizontally as overall data intake increases. Search nodes should have at least 4-8 CPU cores, 16-64GB RAM, and 200GB of disk space or more depending on your logging requirements.
...
A heavy node runs all the sensor components AND Elastic components locally. This dramatically increases the hardware requirements. In this case, all indexed metadata and PCAP are retained locally. When a search is performed through [Kibana](kibana.md), the manager node queries this node's [Elasticsearch](elasticsearch.md) instance. You'll need at minimum 16GB RAM, 4 CPU cores, and 200GB storage. At the bare minimum of 16GB RAM, you will need swap space to avoid issues. We recommend a minimum of 24GB of RAM if you plan on monitoring traffic. The more traffic you plan on monitoring this RAM requirement will also increase.
...
Since receiver nodes only run [Logstash](logstash.md) and [Redis](redis.md), they don't require much CPU or disk space. However, more RAM means you can set a larger queue size for [Redis](redis.md).

## Intrusion Detection Honeypot (IDH) Node

For an [IDH](idh.md) node, the overall system requirements are low: 1GB RAM, 2 CPU cores, 1 NIC, and 100GB disk space.
...
[Suricata](suricata.md) and [Zeek](zeek.md) are very CPU intensive. The more traffic you are monitoring, the more CPU cores you'll need. A very rough ballpark estimate would be 200Mbps per [Suricata](suricata.md) worker or [Zeek](zeek.md) worker. So if you have a fully saturated 1Gbps link and are running [Suricata](suricata.md) for [NIDS](nids.md) alerts and [Zeek](zeek.md) for metadata, then you'll want at least 5 [Suricata](suricata.md) workers and 5 [Zeek](zeek.md) workers. This means you'll need at least 10 CPU cores for [Suricata](suricata.md) and [Zeek](zeek.md) with additional CPU cores for [Full Packet Capture](full-packet-capture.md) and/or other services. If you are monitoring a high amount of traffic and/or have a small number of CPU cores, you might consider using [Suricata](suricata.md) for both alerts and metadata. This eliminates the need for [Zeek](zeek.md) and allows for more efficient CPU usage.
...
The following RAM estimates are a rough guideline and assume that you're going to be running [Suricata](suricata.md) (including full packet capture) and [Zeek](zeek.md) and want to minimize/eliminate packet loss. Your mileage may vary!
...
You'll need some way of getting packets into your sensor interface(s). If you're just evaluating Security Onion, you can replay [pcaps](pcaps.md). For a production deployment, you'll need a SPAN port on an existing switch or a dedicated TAP. We recommend using a dedicated TAP where possible. If collecting traffic near a NAT boundary, make sure you collect from inside the NAT boundary so that you see the true internal IP addresses.
...
-  [APCON](https://www.apcon.com/products)
-  [Arista](https://www.arista.com/)
-  [cPacket](https://cpacket.com)
-  [Garland](https://www.garlandtechnology.com/products)
-  [Gigamon](https://gigamon.com)
-  [KeySight / Ixia / Net Optics](https://www.keysight.com/us/en/cmp/2020/network-visibility-network-test.html)
-  [Profitap](https://www.profitap.com)

### Further Reading

!!! NOTE
    
    For large networks and/or deployments, please also see [https://github.com/pevma/SEPTun](https://github.com/pevma/SEPTun).
