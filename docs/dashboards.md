...
!!! NOTE
    ...
    Check out our Dashboards video at [https://youtu.be/xUBhyF7se8s](https://youtu.be/xUBhyF7se8s)!
...
Dashboards excludes [Cases](cases.md) data by default. If you disable this option, then you can use Dashboards to query your [Cases](cases.md) data.

### Exclude Detections data

Dashboards excludes [Detections](detections.md) data by default. If you disable this option, then you can use Dashboards to query your [Detections](detections.md) data.
...
The easiest way to get started is to click the query drop down box and select one of the pre-defined dashboards. These pre-defined dashboards cover most of the major data types that you would expect to see in a Security Onion deployment: [NIDS](nids.md) alerts from [Suricata](suricata.md), protocol metadata logs from [Zeek](zeek.md) or [Suricata](suricata.md), [Elastic Agent](elastic-agent.md) logs, and firewall logs.
...
Onion Query Language (OQL) starts with standard [Lucene query syntax](https://lucene.apache.org/core/2_9_4/queryparsersyntax.html) and then allows you to add optional segments that control what Dashboards does with the results from the query.
