...
If you need guaranteed message delivery, then you can enable Kafka which replaces [Redis](redis.md) and [Logstash](logstash.md) on the Security Onion Manager node and Receiver nodes.
...
You can modify your Kafka configuration by going to [Administration](administration.md) --> Configuration --> Kafka.
...
Broker configuration can be modified by going to [Administration](administration.md) --> Configuration --> Kafka --> config --> broker.
...
!!! NOTE
    ...
    In order to change the global pipeline you will need to enable the [Administration](administration.md) --> Show advanced settings option.
